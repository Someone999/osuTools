    ...
    /*public partial class OrtdpWrapper
    {
        ...
        public double Calculate(CTBDiffAttributes Attributes, List<OsuGameMod> Mods, int c300, int cTick)
        {
            ...
            if (Attributes.ApproachRate > 9.0)
                approachRateFactor += 0.1 * (Attributes.ApproachRate - 9.0); // 10% for each ApproachRate above 9
            else if (Attributes.ApproachRate < 8.0)
                approachRateFactor += 0.025 * (8.0 - Attributes.ApproachRate); // 2.5% for each ApproachRate below 8

            value *= approachRateFactor;

            if (mods.Any(m => m == OsuGameMod.Hidden))
                // Hiddens gives nothing on max approach rate, and more the lower it is
                value *= 1.05 + 0.075 * (10.0 - Math.Min(10.0, Attributes.ApproachRate)); // 7.5% for each ApproachRate below 10
